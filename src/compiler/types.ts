 ...
 * 
 */
export interface InstructiveNode extends Node, JSDocContainer {
    _statementBrand: any;
}
 ...
 * 
 */
export interface TermQuerylikeNode extends Node, PartialTermQuerylikeNode
{
 ...
 * 
 */
export interface PartialTermQuerylikeNode extends Node
{
    _TermQuerylikeNodeBrand: any;
}
 ...
 * 
 * {@link Expression } is *proper*, *syntactic* *reference*, while
 * - {@link PartialExpression } is not necessarily *proper*
 * - {@link TermQuerylikeNode } is not necessarily *syntactic*
 * 
 */
export interface Expression extends Node, SyntacticNode, PartialExpression, TermQuerylikeNode
{
 ...
 * 
 */
export interface PartialExpression extends Node, SyntacticNode, PartialTermQuerylikeNode
{
    _expressionBrand: any;
}
...
export interface SpreadElement extends PartialExpression {
...
export interface Statement extends Node, SyntacticNode, InstructiveNode, JSDocContainer {
    _statementBrand: any;
}
...
export interface AssertingTypePredicate extends TypePredicateBase, FwdTypePredicateBase
{
    ...
    parameterName ?: never;
    parameterIndex?: never;
    type: BooleanTypePredicativeType ;
}

export interface AssertsThisTypePredicate       extends AssertingTypePredicate { type: ThisTypePredicativeType  ; }
export interface AssertsIdentifierTypePredicate extends AssertingTypePredicate { type: ParamTypePredicativeType ; }

/** 
 * "has done something" ; {@link StatementRunType}
 * 
 */
export interface HasDoneSomethingTypePredicate extends TypePredicateBase, FwdTypePredicateBase
{
...
export type BooleanTypePredicate =
    | ThisTypePredicate
...
export type TypePredicate =
    | BooleanTypePredicate | PostConditioningPredicate
    | ThisTypePredicate | IdentifierTypePredicate
    | AssertsThisTypePredicate | AssertsIdentifierTypePredicate
;
...
export const enum TypeFactFlags1 {
    /** `any` */
    Any             = 1 << 0,
    ...
    /**
     * is a "template" interpolating-string-literal type, or
     * is {@link TypePredicativeType}
     */
    Interpolating = 1 << 27,  // is a "template" interpolating-string-literal type
    /**
     * involves path-dependent type
     */
    PathDependent = 1 << 26,  // involves path-dependent type, or is a "template" interpolating-string-literal type
    /**
...
/**
 * when a `boolean` sub-type is a type-predicate, and additionally it's shown `true` or `false`, or
 * when a `void` sub-type return-value has been obtained (successfully) ,
 ...
 * ```
 * 
 * also occur as {@link Signature.resolvedPreCondition}.
 * 
 */
export interface TypePredicativeType extends Type
{
    mainCond: TypePredicate ;
}
 ...
 * 
 */
export interface BooleanTypePredicativeType extends TypePredicativeType
{
    mainCond: BooleanTypePredicate ;
}

export interface ParamTypePredicativeType extends TypePredicativeType { mainCond: IdentifierTypePredicate ; }
export interface ThisTypePredicativeType extends TypePredicativeType { mainCond: ThisTypePredicate ; }

/**
 * a {@link TypeChecker sub-type} of {@link TypeChecker.getVoidType `void`} (see {@link PostConditionalReturnType} for the non-`void` one!) with added post-cond.
 ...
 * ```
 * 
 * ideally this type cannot legally occur as types of `const`s or *parameter*s ;
 ...
 * 
 * also occur as {@link PostConditionalReturnType.mainThreadType} and {@link Signature.resolvedPreCondition}.
 * 
 */
export interface AssertingVoidKwdType extends TypePredicativeType, PostConditioningVoidKwdType
{
    mainCond: AssertingTypePredicate ;
}
 ...
 * 
 */
export interface PostConditioningVoidKwdType extends TypePredicativeType
{
    mainCond: PostConditioningPredicate ;
}

/**
 * {@link CommaListExpression comma-operator type} `(asserts x is T ; Result )` or `(done x() ; Result )`
 * 
 * essentially the return-type preceded by {@link PostConditioningVoidKwdType `asserts <cond>`} hence `(asserts <cond> ; T )`
 * 
 */
export interface PostConditionalReturnType extends Type
{
    //
    mainThreadType: PostConditioningVoidKwdType ;
    returnType: Type ;
}

/**
 * `type` describing exact sequence of statements
 * 
 * primarily for use as {@link HasDoneSomethingTypePredicate.type } values,
 * otherwise this type might hardly be comparable to anything else
 * 
 */
...
export interface TrivialUnaryType extends Type
{
    //
    subject: Type ;
}
