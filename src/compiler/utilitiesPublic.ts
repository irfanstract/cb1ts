...
export function getNameOfDeclaration(declaration: (ParsedNode & (Declaration | Expression)) | undefined): (ParsedNode & DeclarationName) | undefined ;
export function getNameOfDeclaration(declaration: Declaration | Expression | undefined): DeclarationName | undefined {
 ...
 * @internal
 */
export function isStaticConstantExpressionKind(kind: SyntaxKind): boolean {
    if (SyntaxKind.NumericLiteral <= kind && kind <= SyntaxKind.NoSubstitutionTemplateLiteral) {
        return true ;
    }
 ...
 * 
 */
export function isStaticConstantExpression(node: Expression): boolean {
    if (isStaticConstantExpressionKind(node.kind) ) {
        return true ;
    }
    {
        {
            if ((isArrayLiteralExpression(node)) && (node.elements.length === 0 ) ) {
                return true ;
            }
            if ((isObjectLiteralExpression(node)) && (node.properties.length === 0 ) ) {
                return true ;
            }
        }
        if (isSatisfiesExpression(node) && isStaticConstantExpression(node.expression) ) {
            return true ;
        }
    }
    return false ;
...
// TODO
/**
 * can be shown pure even without typechecking?
 * 
 * @internal
 */
export function isUntypedlyPureExpressionKind(kind: SyntaxKind): boolean {
    if (isStaticConstantExpressionKind(kind) ) {
        return true ;
    }
    {
        ...
        if (kind === SyntaxKind.ThisKeyword || kind === SyntaxKind.SuperKeyword ) {
            return true ;
        }
 ...
 * 
 */
export function isUntypedlyPureExpression(node: Expression): boolean {
    if (isStaticConstantExpression(node) ) {
        return true ;
    }
    if (isUntypedlyPureExpressionKind(node.kind) ) {
        return true ;
    }
    {
        if (node.kind === SyntaxKind.ExpressionWithTypeArguments ) {
            return isUntypedlyPureExpression((node as ExpressionWithTypeArguments).expression) ;
        }
        if (isSatisfiesExpression(node) && isUntypedlyPureExpression(node.expression) ) {
            return true ;
        }
        ...
        {
            if ((isArrayLiteralExpression(node)) && (node.elements.every(isUntypedlyPureExpression ) ) ) {
                return true ;
            }
            // if ((isObjectLiteralExpression(node)) && (node.properties.length.every(isUntypedlyPureExpression ) ) ) {
            //     return true ;
            // }
        }
    }
...
/**
 * Determines whether a node is an {@link Expression} based only on its kind.
 */
...
/**
 * Determines whether a node is an {@link SemiExpression} based only on its kind.
 */
export function isSemiExpression(node: Node): node is SemiExpression {
    return isSemiExpressionKind(skipPartiallyEmittedExpressions(node).kind);
}
...
function isExpressionKind(kind: SyntaxKind): boolean {
    ...
    if (isUnaryExpressionKind(kind) ) {
        return true ;
    }
    return false ;
}

function isSemiExpressionKind(kind: SyntaxKind): boolean {
