...
import {
    ...
    ExportDeclaration,
    ExportSpecifier,
    ExportTypeTDeclarationApparentTypo,
    Expression,
    ExpressionStatement,
    findAncestor,
    FlowAssignment,
    ...
    isExportsIdentifier,
    isExportSpecifier,
    isExportTypeTDeclarationApparentTypo,
    isExpression,
...
const astBinder = /* @__PURE__ */ createSourceFileBinder();

/** @internal */
export function bindSourceFile(file: SourceFile, options: CompilerOptions) {
    performance.mark("beforeBind");
    perfLogger?.logStartBindFile("" + file.fileName);
    astBinder(file, options);
...
function addDeclarationToSymbol1(...[symbol, node, symbolFlags] : [Symbol & DeclaredSymbol, Declaration, SymbolFlags]) : void
{
    {
...
function getRefinedOfDeclarationNameImpl(...[name, node, ] : [DeclarationName , Declaration]): __String | undefined
{
    {
        {
            ...
            if (name.kind === SyntaxKind.ComputedPropertyName) {
                ...
                if (isSignedNumericLiteral(nameExpression)) {
                    ...
                    return Debug.fail("Only computed properties with literal names have declaration names");
                }
            }
...
function getBindableNameOfDeclaration(node: Declaration): __String | undefined {
    {
        ...
        if (name) {
            getRefinedOfDeclarationNameImpl(name, node)
        }
        switch (node.kind) {
            ...
            case SyntaxKind.BinaryExpression:
                ...
                return Debug.fail("Unknown binary declaration kind");
                break;
...
/** Assignment treated as Declaration (eg `this.prop = 1`) */
const isAssignmentDeclaredSymbol: SymbolToFlagsBitwisePredicate = x => isAssignmentDeclaredSymbolFlags(x.flags) ;
/** Assignment treated as Declaration (eg `this.prop = 1`) */
const isAssignmentDeclaredSymbolFlags: SymbolFlagsBitwisePredicate = x => (x & SymbolFlags.Assignment ) ;

/** Assignment treated as Declaration (eg `this.prop = 1`) */
const isVariableSymFlags: SymbolFlagsBitwisePredicate = x => (x & SymbolFlags.Variable ) ;

type SymbolToFlagsBitwisePredicate = (x: Symbol) => SymbolFlagsBitwise ;
type SymbolFlagsBitwisePredicate = (x: SymbolFlagsBitwise) => SymbolFlagsBitwise ;
type SymbolFlagsBitwise = { iSymbolFlagsBitwiseObjVBrand ?: any ; } & SymbolFlags ;
...
}] : [BinderInterComImpl] )
{
    ...
    function addDeclarationToSymbol(symbol: Symbol, node: Declaration, symbolFlags: SymbolFlags) {
        return addDeclarationToSymbol1(symbol, node, symbolFlags)
    }

    // Should not be called on a declaration with a computed property name,
    // unless it is a well known Symbol.
    function getDeclarationName(node: Declaration): __String | undefined {
        return getBindableNameOfDeclaration(node) ;
    }
    ...
    function declareSymbol(symbolTable: SymbolTable, parent: Symbol | undefined, node: Declaration, includes: SymbolFlags, excludes: SymbolFlags, isReplaceableByMethod?: boolean, isComputedName?: boolean): Symbol {
        ...
        if (name === undefined) {
            ...
            if (!symbol) {
                ...
                else if (!(isVariableSymFlags(includes) && isAssignmentDeclaredSymbolFlags(symbol.flags) ) )
                {
...
interface BinderInterComImpl
{
    ...
    addClassifiableName(...args: Parameters<Set<__String>["add"] > ) : any ;

}

function createSourceFileBinder(): BinderOps {
     ...
     * ES6. 
     * if it is not an external module, then
     * we'll determine if it is in strict mode or not depending on
     * if we see "use strict" in certain places or if we hit a class/namespace
    ...
    let symbolPool: ReturnType<typeof createSymbolPool> ;
    let cds : ReturnType<typeof createDeclBinder> ;
    ...
    /**
     * unsets all those variables as if we're done
     * 
     */
    ...
    function bindSourceFile(f: SourceFile, opts: CompilerOptions) {
        ...
        cds = createDeclBinder(bdIntercom) ;
    ...
    function addClassifiableName(...args: Parameters<typeof classifiableNames.add> )
    { return classifiableNames.add(...args) ; }
    ...
    function addDeclarationToSymbol(symbol: Symbol, node: Declaration, symbolFlags: SymbolFlags) {
        return cds.addDeclarationToSymbol(symbol, node, symbolFlags) ;
    }

    // Should not be called on a declaration with a computed property name,
    // unless it is a well known Symbol.
    function getDeclarationName(node: Declaration): __String | undefined {
        return cds.getDeclarationName(node) ;
    }

    function getDisplayName(node: Declaration): string {
        return cds.getDisplayName(node) ;
    }
     ...
     * @param excludes - The flags which node cannot be declared alongside in a symbol table. Used to report forbidden declarations.
     */
    function declareSymbol(...args: [symbolTable: SymbolTable, parent: Symbol | undefined, node: Declaration, includes: SymbolFlags, excludes: SymbolFlags, isReplaceableByMethod?: boolean, isComputedName?: boolean] ): Symbol {
        ;
        return cds.declareSymbol(...args) ;
    }
    ...
    function jsdocTreatAsExported(node: Node & SyntacticNode & ParsedNode)
    {
    ...
    function isNarrowingExpression(expr: Expression): boolean {
        ...
        if (isStaticConstantExpression(expr) || isUntypedlyPureExpression(expr) ) {
            return false ;
        }
        return true;
    }
    ...
    function createBranchLabel(): FlowLabel {
        return initFlowNode({ flags: FlowFlags.BranchLabel, antecedents: undefined } satisfies FlowLabel);
    }

    function createLoopLabel(): FlowLabel {
        return initFlowNode({ flags: FlowFlags.LoopLabel, antecedents: undefined } satisfies FlowLabel);
    }

    function createReduceLabel(target: FlowLabel, antecedents: FlowNode[], antecedent: FlowNode): FlowReduceLabel {
        return initFlowNode({ flags: FlowFlags.ReduceLabel, target, antecedents, antecedent } satisfies FlowReduceLabel);
    }
    ...
    function setFlowNodeReferenced(flow: FlowNode) {
        // On first reference we set the Referenced flag, thereafter we set the Shared flag
        flow.flags |= (
            flow.flags & FlowFlags.Referenced ? FlowFlags.Shared : FlowFlags.Referenced
        ) ;
    }
     ...
     * 
     */
    function addAntecedent(...[label, antecedent,] : [receiver: FlowLabel, ...args: [addend: FlowNode, ] ] ): void
    {
    ...
    function createFlowCondition(flags: FlowFlags, antecedent: FlowNode, expression: (Expression & ParsedNode) | undefined): FlowNode
    {
        ...
        setFlowNodeReferenced(antecedent);
        return initFlowNode({ flags, antecedent, node: expression } satisfies FlowCondition );
    }

    function createFlowSwitchClause(antecedent: FlowNode, switchStatement: SwitchStatement, clauseStart: number, clauseEnd: number): FlowNode {
        setFlowNodeReferenced(antecedent);
        return initFlowNode({ flags: FlowFlags.SwitchClause, antecedent, switchStatement, clauseStart, clauseEnd } satisfies FlowSwitchClause );
    }
    ...
    function createFlowMutation(flags: FlowFlags, antecedent: FlowNode, node: Expression | VariableDeclaration | ArrayBindingElement): FlowNode {
        setFlowNodeReferenced(antecedent);
        const result = initFlowNode({ flags, antecedent, node } satisfies FlowAssignment);
    ...
    function createFlowCall(antecedent: FlowNode, node: CallExpression): FlowNode {
        setFlowNodeReferenced(antecedent);
        return initFlowNode({ flags: FlowFlags.Call, antecedent, node } satisfies FlowCall);
    }
    ...
    function isStatementCondition(node: Node & ParsedNode)
    {
    ...
    function isTopLevelLogicalExpression(node: SyntacticNode & ParsedNode): boolean {
        while (
    ...
    function bindDestructuringTargetFlow(node: Expression | OmittedExpression) {
    ...
    function bindAssignmentTargetFlow(node: Expression | OmittedExpression) {
        if (isExpression(node) && isNarrowableReference(node)) {
    ...
    function createBindBinaryExpressionFlow(): any {
        // TODO
    }
    ...
    /**
     * 
     * misnamed alias of {@link bindAsChild}.
     * 
     * @deprecated please dealias.
     * 
     */
    function bind(...args: Parameters<typeof bindAsChild>)
    {
        return bindAsChild(...args) ;
    }

    function bindAsChild(node: Node | undefined): void {
        if (!node) {
            return;
        }
    ...
    /**
     * run one of the `bindYyy` methods which one depending on the {@link SyntacticNode}'s `kind`.
     * 
     */
    function bindNamesOf(node: Node & SyntacticNode & ParsedNode)
    {
        switch (node.kind) {
            /* Strict mode checks */
            case SyntaxKind.Identifier:
    ...
    //
    /**
     * run one of the `bindYyy` methods which one depending on the {@link TypeNode}'s `kind`.
     * 
     */
    function bindTypeNode(node: Node & TypeNode & SyntacticNode & ParsedNode)
    {
        // TODO
    }
    ...
    function bindThisPropertyAssignment(node: BindablePropertyAssignmentExpression | PropertyAccessExpression | LiteralLikeElementAccessExpression) {
        ...
        switch (thisContainer.kind) {
            case SyntaxKind.FunctionDeclaration:
            case SyntaxKind.FunctionExpression:
                let constructorSymbol: DeclaredSymbol | undefined = thisContainer.symbol;
    ...
    function addLateBoundAssignmentDeclarationToSymbol(node: BinaryExpression | DynamicNamedDeclaration, symbol: DeclaredSymbol | undefined) {
        if (symbol) {
    ...
    function bindObjectDefinePrototypeProperty(node: BindableObjectDefinePropertyCall) {
        const namespaceSymbol = lookupSymbolForPropertyAssignmentDeclaration((node.arguments[0] as PropertyAccessExpression).expression as EntityNameExpression);
    ...
    function bindPotentiallyMissingNamespaces(namespaceSymbol: DeclaredSymbol | undefined, entityName: BindableStaticNameExpression, isToplevel: boolean, isPrototypeProperty: boolean, containerIsClass: boolean) {
    ...
    function isTopLevelNamespaceAssignment(propertyAccess: ParsedNode & BindableAccessExpression)
    {
     ...
     */
    function isExpandoSymbol(symbol: DeclaredSymbol): boolean
    {
    ...
    function getParentOfBinaryExpression(expr: ParsedNode)
    {
    ...
    // TODO
    function lookupSymbolForPropertyAssignmentDeclaration(...args: Parameters<typeof lookupSymbolForPropertyAccess>) : DeclaredSymbol | undefined
    { return lookupSymbolForPropertyAccess(...args) ; }
