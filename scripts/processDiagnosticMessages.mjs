...
function buildInfoFileOutput(messageTable, inputFilePathRel) {
    return stringByLinesItr(function* () {
    ;

    yield* [
        "// <auto-generated />",
        `// generated from '${inputFilePathRel}' by 'scripts/processDiagnosticMessages.mjs' - modify the JSON file instead`,
        "",
    ...
    for (const [message, { code, category, reportsUnnecessary, elidedInCompatabilityPyramid, reportsDeprecated }] of messageTable.entries() )
    {
        const propName = convertPropertyName(message);
        ...
        const docString = (
            `diagnostic item ${asBackticked(code) }, *${category }* ${asBackticked(message) }`
        ) ;

        yield `    ${asJsDoc(docString) } ` ;
        yield `    ${propName}: diag(${code}, DiagnosticCategory.${category}, "${createKey(propName, code)}", ${JSON.stringify(message)}${argReportsUnnecessary}${argElidedInCompatabilityPyramid}${argReportsDeprecated}),` ;
    }
